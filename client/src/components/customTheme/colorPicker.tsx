...
const ColorPicker = ({ keyType, onChange, givenColor }: { keyType: any, onChange: (key: any, color: string) => void, givenColor: string }) => {
  const [color, setColor] = useState(givenColor)

  const accent_color = getStyle()?.accentColor || "#6488ea"

  const bg_color_chakra = useColorModeValue("#fff", "#222731")
