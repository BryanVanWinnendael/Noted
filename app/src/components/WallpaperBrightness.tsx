  ...
  return (
    <Flex justifyContent="space-between" alignItems="center" mr={5} mt={2}>
      <Box>
        ...
        <Text color={muted_text_color}>Choose the brightness for the background</Text>
      </Box>

      <Flex w="50%" gap={2} alignItems="center">
        <Slider value={brightnessValue} aria-label='slider-ex-1' defaultValue={36} onChange={(val) => handleChange(val)}>
          <SliderTrack>
            <SliderFilledTrack />
          </SliderTrack>
          <SliderThumb />
        ...
        h={8}
        bg={accent_color}
         onClick={() => handleChange(100)}>Reset</Button>
      </Flex>
    </Flex> 
  )
}
